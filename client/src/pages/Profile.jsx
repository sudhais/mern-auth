...
import { updateUserStart, updateUserFailure, updateUserSuccess, deleteUserFailure,deleteUserStart,deleteUserSuccess, signoutUser } from "../redux/user/userSlice";
...
export default function Profile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`api/v1/user/update/${currentUser.user._id}`, {
        method: 'POST',
        headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const res = await fetch(`api/v1/user/delete/${currentUser.user._id}`, {
        method: 'DELETE'
      });
      const data = await res.json();
      console.log(data);
  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      <p className='text-green-700 mt-5'>{success && 'Successfully updated'}</p>
    </div>
  )
}
